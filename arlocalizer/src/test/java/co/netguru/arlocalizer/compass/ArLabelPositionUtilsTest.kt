...
import co.netguru.arlocalizer.arview.ARLabelPositionUtils
...
class ArLabelPositionUtilsTest {
    ...
    fun `for maximum Angle range end positionX on the display should be in the center (half of width)`() {
        ...
        val positionX = ARLabelPositionUtils.calculatePositionX(
            compassData,
            displayWidth
        )
    ...
    fun `for maximum angle range start positionX should be reaching start of display`() {
        val compassData = CompassData().apply {
            currentDestinationAzimuth =
                ARLabelPositionUtils.HORIZONTAL_ANGLE_RANGE_MAX.start
        }
    ...
    fun `low pass filter alpha should have the precise value at the center of the screen`() {
        ...
        val lowPassFilterAlpha =
            ARLabelPositionUtils.adjustLowPassFilterAlphaValue(
                centerPosition,
                displayWidth
            )
    ...
    fun `low pass filter alpha should be in range of normal and precise`() {
        ...
        assert(lowPassFilterAlpha in ARLabelPositionUtils.LOW_PASS_FILTER_ALPHA_NORMAL
                ..ARLabelPositionUtils.LOW_PASS_FILTER_ALPHA_PRECISE)
    }
    ...
    fun `for minimum angle range end positionY on the display should be in the center (half of height)`() {
        val compassData = CompassData().apply {
            orientationData = OrientationData(
                0f,
                ARLabelPositionUtils.VERTICAL_ANGLE_RANGE_MIN.endInclusive
            )
        }

        val positionY = ARLabelPositionUtils.calculatePositionY(
            compassData,
            displayHeight
        )
    ...
    fun `for maximum angle range start positionY on the display should be in the center (half of height)`() {
        val compassData = CompassData().apply {
            orientationData = OrientationData(
                0f,
                ARLabelPositionUtils.VERTICAL_ANGLE_RANGE_MAX.start
            )
        }
    ...
    fun `for maximum angle range end positionY on the display should be at the top`() {
        val compassData = CompassData().apply {
            orientationData = OrientationData(
                0f,
                ARLabelPositionUtils.VERTICAL_ANGLE_RANGE_MAX.endInclusive
            )
        }
    ...
    fun `for minimum angle range start positionY on the display should be at the bottom`() {
        val compassData = CompassData().apply {
            orientationData = OrientationData(
                0f,
                ARLabelPositionUtils.VERTICAL_ANGLE_RANGE_MIN.start
            )
        }
